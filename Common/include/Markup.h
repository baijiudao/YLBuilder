...
public:
    ...
    bool Load(MCD_PCSZ pcszFileName);//add by Yingjie
	bool SetDoc( MCD_PCSZ pDoc );
	bool SetDoc( const MCD_STR& strDoc );
	bool IsWellFormed();
	bool FindElem( MCD_CSTR szName);
    bool FindElem(MCD_PCSZ pcszName = NULL);//add by Yingjie
    ...
    MCD_STR GetAttrib(MCD_PCSZ pcszAttrib) const { MCD_CSTR szAttrib; szAttrib.pcsz = pcszAttrib; return GetAttrib(szAttrib); };//add by Yingjie
	...
	bool Save(MCD_PCSZ pcszFileName){ MCD_CSTR szFileName(pcszFileName); return Save(szFileName); };//add by Yingjie
	...
	bool AddElem( MCD_PCSZ pcszName, MCD_CSTR szData = NULL, int nFlags = 0) { MCD_CSTR szName(pcszName); return AddElem(szName, szData, nFlags); };//add by Yingjie
	...
	bool AddAttrib( MCD_PCSZ pcszAttrib, MCD_PCSZ pcszValue ) { MCD_CSTR szAttrib(pcszAttrib), szValue(pcszValue); return AddAttrib(szAttrib, szValue); };//add by Yingjie
