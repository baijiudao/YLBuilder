    ...
    if(inItWebMode.editMode == true){
        switch(val){
            ...
            case 'communicate':
                ...
                break;
            case 'removalshit':
                inItRemovalshitcateControl.createRemovalshitControl();
                var alarmLog = inTtCommand.createLog();
                webapi.addLog('create',alarmLog);
                break;
            case 'video':
...
var setJsonInfo = function(url,jsonStr){ //参数：url：路径、jsonStr：json型字符串
    ...
    var urlT = $("#inputUrl");
    urlT.val(url);
    variableCfgO.empty(); //已经记录了上次配置文件路径，如果重新点击【浏览】选择配置文件，清空节点variableCfgO
    cfg.appendTo(variableCfgO);
    var oSunsys;
    if (jsonO["Subsys"]) {
        oSunsys = jsonO["Subsys"];
        variableInfoConfig(oSunsys, variableCfgO, url); //控件变量配置
    } else if (jsonO["Camera"]) {
        oSunsys = jsonO["Camera"];
        videoInfoConfig(oSunsys, variableCfgO, url); //通用视频控件视频信息配置
    }
};
var variableInfoConfig = function(oSunsys, variableCfgO, url) {
        ...
        $.each(oSunsys[k]["Device"],function(v){
            ...
            $.each(oSunsys[k]["Device"][v]["Tag"],function(g){
                var variableO = $('<div class="variable hide" id="variable'+k+v+g+'">'+
					...
					'DataBaseProp='+oSunsys[k]["Device"][v]["Tag"][g]["DataBaseProp"]+' ' +
    ...
    localStorage.setItem('navHtml', variableCfgO.html());
    localStorage.setItem('jsonUrl', url);
};
var videoInfoConfig = function(oSunsys, variableCfgO, url) {
    $.each(oSunsys,function(k){
        var system = $('<div class="systems hide" id="systems'+k+'">'+
            '<img class="systemsPic font_attr" src="images/close.png" />'+
            '<div class="systemsTxt font_attr videoConfig" >'+oSunsys[k]["URL"]+'</div>'+
            '</div>');
        system.appendTo(variableCfgO);
        var id, rtspAddress, outputPort, inputPort, videoWidth, videoHeight;
        id = oSunsys[k]['Id'];
        rtspAddress = oSunsys[k]["URL"];
        outputPort = oSunsys[k]['OutputPort'];
        inputPort = oSunsys[k]['InputPort'];
        videoWidth = oSunsys[k]['VideoWidth'];
        videoHeight = oSunsys[k]['VideoHeight'];
        $("#systems"+k).children("div").attr({
            "id": id,
    ...
    localStorage.setItem('videoInfoHtml', variableCfgO.html());
    localStorage.setItem('videoJsonUrl', url);
};
...
window.onload = function() {
    new QWebChannel(qt.webChannelTransport, function(channel) {
        ...
        webapi.getSvgPath = webedit.getSvgPath;
