...
var AllElementId = function(){
    ...
    this.hkvideoElementIDMaxNum = function(){
        var arrNum = [];
    ...
    this.pipElementIDMaxNum = function(){
        var arrNum = [];
        $.each($("div[id^='Pip']"),function(){
            arrNum.push(parseInt($(this).attr("id").split("_")[1]));
        });
        return Math.max.apply(null, arrNum);
    };
    ...
    if(inItWebMode.editMode == true) {
        ...
        switch (ev.keyCode) {
            ...
            case 46:
                ...
                break;
            case 38:        
                 inItPipelineControl.pip_config_(selecteId);//管道控件配置联动前
                break;
            case 40:        
                inItPipelineControl.pip_config(selecteId);//管道控件配置联动后
                break;
            case 8:        
                inItPipelineControl.pip_config_delet(selecteId);//管道控件配置联动删除
                break;
        }
...
//功能：显示控件的样式；
    this.setStyle = function(selectedArr){ //参数：selectedArr；选择集id；
        ...
        if(len == 1){
			if(selectedArr[0].split("_")[0] != "pic"){
				...
				$("#"+selectedArr[0]).addClass("boder").css({"z-index":"1"}).children('.hiden').css({
					"border-color":"#39bbf6",
					"background":"#ffffff"
				});
				$("#"+selectedArr[0]).siblings(".contrl").removeClass("boder").css("z-index","2").children('.hiden').remove();
...
var Show = function() {
    ...
    /*===============同步字体===============*/
    $(".exp"+idd).css("font-family",$("#"+idd).attr("fontfaml"));
    $("#"+idd).children("div").css("font-family",$("#"+idd).attr("fontfaml"));
...
var ModalFeature = function(){ //模态框定义
    ...
    this.Modalbox = function(id){   //显示模态框
        ...
        ConfigO.bind("click",function(){
            ...
            if(isOpened == true){
                ...
                $('#inputUrl').val(localStorage.getItem('jsonUrl'));
            }
    ...
    this.menuOperate = {
        variableMsg:function(idd){ //显示变量相关属性
            ...
            $("#variableCfg").on("click",".variableTxt",function(){
                $(this).addClass('variableTxtHover').parents('.variable').siblings().children('.variableTxt').removeClass('variableTxtHover');
                $(this).parents('.device').siblings().children('.variable').children('.variableTxt').removeClass('variableTxtHover');
                var control = $("#"+idd);
                var sureBtn = $("#sureBtn");
                ...
                var Name ='',ID = '',DataType = '',DataComment = '',MinEuVal = '',MaxEuVal = '';
					...
					DataBaseProp = $(this).attr("DataBaseProp");
                    sureBtn.removeAttr("support");
                key.each(function(i){
                    switch(i){
                        case 0:
                        ...
                        case 1:
                            key.eq(1).text("变量ID");
                            val.eq(1).text(ID).attr("DataBaseProp",DataBaseProp);
                            break;
                        case 2:
                            key.eq(2).text("变量类型");
    ...
    this.bindvariable = function(idd){ //对控件绑定变量id
        ...
        sureBtn.bind("click",function(){ //对控件添加变量的ID属性
			...
			var DataBaseProp = oParent.children(".variableId").attr("DataBaseProp");
            ...
            if(sureBtn.attr("support") == "support"){
                ...
                if(idd[0].split("_")[0] == "Chart"){//对图表控件的配置多变量
					...
					controlO.attr("DataBaseProp",DataBaseProp);
    ...
    this.orderControlFeatures = function(confir, succe, fail, idd){
        ...
        function confrimOrder() { //确认控制命令
            confir.bind('click', function () {
                if (buttonoff1) {
                    controlObj.attr("sure", "yes");
                    $(this).css({
                        'background-image': 'url(images/selected.png)'
                    });
                } else {
                    controlObj.attr("sure", "no");
                    $(this).css({
        ...
        function successOrder() { //报告成功命令
            succe.bind('click', function() {
                if (buttonoff2) {
                    controlObj.attr("success","yes");
                    $(this).css({
                        'background-image': 'url(images/selected.png)'
                    });
                } else {
                    controlObj.attr("success","no");
                    $(this).css({
                        'background-image': 'url(images/notselected.png)'
                    });
                }
        ...
        function failOrder() { //报告失败命令
            fail.bind('click', function() {
                if (buttonoff3) {
                    controlObj.attr("fails","yes");
                    $(this).css({
                        'background-image': 'url(images/selected.png)'
                    });
                } else {
                    controlObj.attr("fails","no");
                    $(this).css({
                        'background-image': 'url(images/notselected.png)'
                    });
                }
