...
if (inItWebMode.editMode == false) {
    ...
    var sendPostData = function(val, obj) { //post方式 下发命令数据格式  val为值   obj为控件idd标识
        packetIDCount = packetIDCount + 1;
        var sendDataList = {};
        sendDataList["RequestMethod"] = 'post';
        sendDataList["packetID"] = packetIDCount;
    ...
    var host = "ws:192.168.100.6:32767";
        ...
        if (msg.data != "") {
            ...
            case 0:
                ...
                case 0:
                    ...
                    //document.cookie = "Session_value="+Session_value;
    ...
    var colseSocket = function() { //关闭socket连接
...
var addtemplate = function() {
    if (inItWebMode.editMode == false) {
        ...
        var rightFrame = $('#right', window.parent.document).contents().find(".right");
...
300);
	//登陆、退出操作处理
	var logFun = function(){
		var loginBtn = $('#top', window.parent.document).contents().find(".log");
		var logoutBtn = $('#top', window.parent.document).contents().find("#out");
		var hided = $('#top', window.parent.document).contents().find(".hided");
		var passIput = $('#top', window.parent.document).contents().find(".pass");
		var userIput = $('#top', window.parent.document).contents().find(".text");
		
		//登录退出
		loginBtn.bind("click",function(){//登录
			var pass = $('#top', window.parent.document).contents().find("#passWord").val();
			var user= $('#top', window.parent.document).contents().find("#userName").val();
			if(pass != '' && user != ''){
		...
		//默认工作区的大小,背景；
		var rightFrame = $('#right', window.parent.document).contents().find(".right");
			rightFrame.height($("#area",parent.document.body).height());
			rightFrame.width(100+'%');
			rightFrame.css("background","url(images/frameBg.png) no-repeat 100% 100%");
	}
	setTimeout(function(){
		logFun();
	},1000);
