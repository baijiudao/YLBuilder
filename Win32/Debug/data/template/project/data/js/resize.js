...
var Bind = function(object, fun) {//添加方法
	return function() {
...
var BindAsEventListener = function(object, fun) {
	var args = Array.prototype.slice.call(arguments).slice(2);//把类数组对象转换成一个真正的数组
	return function(event) {
		return fun.apply(object, [event || window.event].concat(args));
	}
}

var CurrentStyle = function(element){//读取css样式属性
	return element.currentStyle || document.defaultView.getComputedStyle(element, null);
}

function addEventHandler(oTarget, sEventType, fnHandler) {//添加监听事件
...
function removeEventHandler(oTarget, sEventType, fnHandler) {//移除监听事件
...
Drag.prototype = {
	...
	Move: function(oEvent) {
		...
		else if (iLeft >= document.documentElement.clientWidth - this.Drag.offsetWidth) {
			iLeft = document.documentElement.clientWidth - this.Drag.offsetWidth;
		}
		if (iTop <= 0) {
			iTop = 0;
		}
		else if (iTop >= document.documentElement.clientHeight - this.Drag.offsetHeight) {
			iTop = document.documentElement.clientHeight - this.Drag.offsetHeight;
		}
